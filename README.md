...
인터페이스 역할을 할 태블릿과 카메라, 라즈베리파이 설치를 위해 정면에서 보이지 않는 위치에 거치대를 구조하고 단단하게 고정할 수 있도록 도면을 설계했다.

기구물은 전기차 충전기를 소형화한 것이므로 실내 뿐만 아니라 야외에서도 차질 없이 사용되어야 하는 점을 고려, 밑면과 윗면에 넓은 면적의 지지대를 부착해 넘어짐을 방지했다.

외부 충격에도 파손되지 않아야 하고 데모 영상 촬영과 전시회를 위해 이동에 용이한 가벼운 재질이어야 함. 그에 적합한 폴리카보네이트(PC)를 사용해 구조물을 제작했음.  
